...
asvs <- read_tsv("data/summed_asvs_filt_test.tsv", col_types =cols(
  asv = col_character(),
  sample = col_character(),
  count = col_double(),
  relabraw = col_double(),
  contaminant_negfilt = col_logical(),
  pa.pos = col_double(),
  pa.neg = col_double(),
  contaminant_decDef = col_logical(),
  contaminant_prev05 = col_logical(),
  contaminant_prev055 = col_logical(),
  relab_negfilt = col_double(),
  relab_decDef = col_double(),
  relab_Dec05 = col_double(),
  relab_dec055 = col_double() ))
...
asvs <- asvs %>% inner_join(metadata %>% select(sample,  Asample)) %>%
  filter(contaminant_prev05 == FALSE) %>% select(asv, sample, count)
