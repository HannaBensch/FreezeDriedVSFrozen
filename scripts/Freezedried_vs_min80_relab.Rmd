...
# keep ASVs counts part of Freeze-dried frozen comparison
asvs <- asvs %>% inner_join(metadata %>% dplyr::select(sample), by ="sample") %>% select(-hellinger)
...
# filter taxonomy to this study
taxonomy <-taxonomy %>% inner_join(asvs %>% select(asv) %>% unique())
```
...
asvs %>% left_join(taxonomy) %>% group_by(sample, phylum) %>% summarise(relab = sum(relab)) %>% filter(phylum =="Proteobacteria") %>% arrange(desc(relab)) 
...
```{r Figure 2A, echo=FALSE, fig.cap='**Correlation relative abundance within sample by sample treatment on n ASV level.** Facet by phylum, colour by top 10 phyla.', fig.height=12, fig.width=12, warning=FALSE, message=FALSE}
...
asvs %>%
  ...
  stat_cor(aes(label = paste(..r.label.., ..p.label..,sep = "~`,`~")), size  = 2) +
...
topfam <- asvs %>% 
  ...
  mutate(Order  = as.double( 1:nrow(.)),
         topph = case_when( Order == 17 ~ "family < 1 % abundance",
                           TRUE ~ family))
...
asvs %>%
  ...
  left_join(metadata %>% select(sample, Treatment, SampleNumber, SampleOrder)) %>%
  left_join(topfam %>% dplyr::select(Order, topph)) %>% 
  ...
  geom_col(position = 'fill') +
  scale_fill_manual(labels = fooBarLabels, values = wes_palette("Darjeeling1", 17, type = "continuous")) +
  coord_flip() +
      theme(axis.text.x = element_text( size =15),
...
asvs %>%
  ...
  pivot_longer(2:18, names_to = 'topph', values_to = 'relab') %>%
...
```{r diffFdFs, fig.width=12, fig.height = 12, fig.cap = '**qqplot family level relab. **', message=FALSE, echo=FALSE, warning=FALSE}
